...
- `services/questionGenerationService.js` — the *only* place allowed to call an AI API for question generation. Keep it provider-agnostic (adapter shape) so Claude/Gemini/OpenAI/local model can be swapped without touching the bank.
