...
- "content"(이번 달 학습 내용)는 과목별 객체로 반환하세요. 메모는 "[과목명]" 뒤에 그 과목의 학습 기록이 줄줄이 나열된 형태로 주어집니다 — 각 과목마다, 배운 항목을 하나하나 전부 나열하지 말고 비슷한 것끼리 묶어서 큰 흐름으로 요약한 문장을 그 과목명 키의 값으로 넣으세요.
  예: [문법] 메모에 "명사의 인칭 구분, be동사, 일반동사, 의문사, 선택의문문, 부가의문문, 명령문, 청유문, 감탄문"이 있다면
  → content.문법 = "인칭 구분과 다양한 동사, 그리고 의문사·명령문 등 다양한 문장 형태에 대해 학습했습니다" 처럼 상위 개념으로 묶어서 표현하세요.
  메모에 없던 과목은 content 객체에 아예 키를 만들지 마세요. 메모 전체가 "(메모 없음)"이면 content는 빈 객체 {}로 반환하세요.
...
// "이번 달 학습 내용"은 (2026-09-01부터) 과목별 객체({"문법": "...", ...})로 넘어온다 — 예전엔
// 문자열 하나였다. 그대로 템플릿 리터럴에 넣으면 "[object Object]"로 문자열화되어 AI가 실제
// 학습 기록을 전혀 못 보고 다듬는 버그가 있었다(실사용 중 재현: AI가 만든 문자열이 결과
// content로 그대로 들어가면서, 화면은 그 문자열을 다시 과목별 객체인 것처럼 글자 단위로
// 쪼개 렌더링해 완전히 깨져 보였다). 과목별로 "[과목명]\n내용" 블록을 풀어서 넣어준다.
function formatReportContentForPrompt(content) {
  if (!content || typeof content !== "object") return content ? String(content) : "(메모 없음)";
  const entries = Object.entries(content).filter(([, v]) => v && String(v).trim());
  if (entries.length === 0) return "(메모 없음)";
  return entries.map(([cat, text]) => `[${cat}]\n${text}`).join("\n\n");
}
...
- 이번 달 학습 내용 (과목별):
${formatReportContentForPrompt(r.content)}
- 칭찬할 점: ${r.good || "(메모 없음)"}
- 보완할 점: ${r.improve || "(메모 없음)"}
- 선생님 종합 코멘트: ${r.comment || "(메모 없음)"}
- 다음 달 학습 방향: ${r.nextMonth || "(메모 없음)"}

위 메모들을 각각 다듬어서 JSON 스키마 형식으로만 답하세요. "(메모 없음)"으로 표시된 항목은(이번 달 학습 내용은 빈 객체 {}로, 나머지는 빈 문자열로) 그대로 반환하세요.`;
}
